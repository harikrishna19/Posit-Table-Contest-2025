...
inner_table<-list("Incoming Transfers"=calculated_cols(inner_table$Incoming),
                   "Outgoing Transfers"=calculated_cols(inner_table$Outgoing))
```
...
# ),
  ...
  pagination = F,
     ...
     defaultSorted = "Expenditure",   # 👈 sort this column on load
     defaultSortOrder = "desc",        # 👈 initial order (asc / desc)
      ...
      columns = list(
        team_logo = colDef(name="",
                           cell = embed_img(width=40,height=40)
        ),
        ...
        Total_Spent_Convert.y = colDef(minWidth = 150,
          ...
          cell = data_bars(
            ...
            text_position = "outside-end",   # <- completely to the right
            text_color = "black",
            fill_color = "#2ecc71", # ensure readable text
            bar_height = "20px",
            number_fmt = scales::label_number(suffix = "M")
          )
        ),
